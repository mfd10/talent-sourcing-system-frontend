...
export default function EditCandidate() {
  ...
  const onSubmit= async (e) => {
        e.preventDefault();
        await axios.put(`http://localhost:8080/candidate/candidates/${candidate.id}`, candidate)
        navigate("/")

  }
  const loadCandidate= async () => {
    const result= await axios.get(`http://localhost:8080/candidate/candidates/${candidate.id}`);
